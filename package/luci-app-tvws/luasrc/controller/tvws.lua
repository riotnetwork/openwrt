...
module("luci.controller.tvws", package.seeall)

function index()
        if not nixio.fs.access("/etc/config/tvws") then -- look at /etc/config/mixer for current config
                return
        end

        local page =  entry({"admin", "network", "tvws"}, cbi("tvws"), _("TVWS"))        -- create "Configuration" tab
        page.dependent = true

        entry({"admin", "network", "tvws_status"}, call("tvws_status"))        -- create "Status" section, but its a callback to the htm template in the cbi
end



function tvws_status()
